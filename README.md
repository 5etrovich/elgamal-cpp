...
Срок выполнения работы: с 2025-12-08 по 2025-12-14.
Срок сдачи работы: 2025-11-14.
...
#### Общее задание
На основе класса `BigNum` из ЛР-6 разработать класс `ElGamal`
в котором реализовать следующий интерфейс:
  * `EGKeyPair genpkey() const`, где `EGKeyPair` задается следующим
    образом `struct EGKeyPair{BigNum public, private, modulus, generator;}`;
  * `void setpkey(const EGKeyPair& p)`;
...
Для `EGKeyPair` реализовать операции помещения в поток и
извлечения из потока.

В основной функции продемонстрировать разработанный функционал создав и
инициализировав объект класса `ElGamal`, вывести ключевую пару на экран,
